<script lang="ts">
		...
		if (isTMA()) {
			try {
				if (shareURL.isAvailable()) {
					shareURL(gameUrlForTG, shareText);
				}
			} catch (error) {
				console.error("Failed to share via Telegram:", error);
				// Fallback to clipboard
				fallbackToClipboard(shareText, gameUrl);
			}
		} else {
	...
	function fallbackToClipboard(shareText: string, gameUrl: string) {
		navigator.clipboard
			.writeText(`${shareText}\n${gameUrl}`)
			.then(() => {
...
<div class="relative mt-14">
	...
	<div class="card bg-base-100 bg-primary rounded-3xl mt-14">
		...
		<div
			class="flex flex flex-col items-center text-primary-content text-center h-full p-4"
		>
			{#if quizState.result}
				<h2 class="text-5xl font-bold mb-6 mt-4 text-main-font">
					"{quizState.gameName}"
				</h2>
				<div
					class="w-40 h-40 md:w-48 md:h-48 mask mask-squircle bg-gradient-to-b from-black/50 to-primary flex flex-col items-center justify-center mb-6"
				>
					<!-- Score display -->
					<div class="text-center">
						<div
							class="text-4xl md:text-5xl font-bold text-main-font"
						>
							{quizState.result.totalScore}
						</div>
						<div>
							<span
								class="text-xl md:text-2xl font-bold text-main-font"
								>из</span
							>
							<span
								class="text-2xl md:text-3xl font-bold text-main-font"
								>{quizState.progress.total}</span
							>
						</div>
					</div>
				</div>
